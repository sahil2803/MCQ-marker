...
def auto_rotate_image(image):
    """Automatically rotate image to correct orientation using PyTesseract"""
    try:
...
def load_image(image_path):
    """Load and prepare image for processing with auto-rotation"""
...
def detect_hough_circles(gray):
    """Detect all bubbles using Hough Circle Transform"""
...
def detect_contour_filled(gray, section_bounds):
    """Detect filled bubbles using contour analysis in a specific section"""
    contour_filled = []
    ...
    for shape in contours:
        ...
        if 170 < area < 345:  # Filled bubble size range
...
def combine_detections(hough_bubbles, contour_filled, tolerance=5):
    """Combine both detection methods with consensus building"""
    combined_bubbles = []
...
def organize_into_questions(combined_bubbles, column_bounds):
    """Organize bubbles into questions and determine answers"""
    all_answers = {}
    question_details = {}
...
def organize_student_number(combined_bubbles, student_section):
    """Organize student number bubbles into the specific format: number,number,letter,number,number,number,number
    Each position has bubbles arranged vertically from 0-9 (or A-Z for the letter position)"""
    
    x, y, w, h = student_section['x'], student_section['y'], student_section['w'], student_section['h']
    
    # Find bubbles in student number section
    student_bubbles = []
    for bubble in combined_bubbles:
        bx, by = bubble['center']
        if x <= bx <= x + w and y <= by <= y + h:
    ...
    # Group bubbles by column (each digit/letter position has its own column)
    ...
    for i, (col_x, bubbles) in enumerate(sorted_columns):
        ...
        # Sort bubbles in column by y-coordinate (top to bottom)
...
def visualize_results(img, combined_bubbles, all_answers, question_details, student_section=None, student_number=None, student_details=None):
    ...
    if student_section and student_details:
        ...
        # Label student number characters and bubbles
    ...
    if student_number:
        ...
        cv2.putText(img, f"Student Number: {student_str}", (x, y + h + 30), 
                   cv2.FONT_HERSHEY_SIMPLEX, 0.7, COLORS['text'], 2)
    
    # Add legend
    legend_y = 30
    legend_items = [
        ("Red: Both methods (HIGH confidence)", COLORS['both_methods']),
        ("Green: Contour only (MEDIUM confidence)", COLORS['contour_only']),
        ("Orange: Empty bubbles", COLORS['empty_bubble']),
        ("Cyan: Hough only", COLORS['hough_only']),
        ("Magenta: Student Number Section", COLORS['student_section'])
    ]
    
    for i, (text, color) in enumerate(legend_items):
        y_pos = legend_y + (i * 25)
        cv2.rectangle(img, (10, y_pos - 10), (30, y_pos + 10), color, -1)
        cv2.putText(img, text, (40, y_pos + 5), 
...
def generate_report(all_answers, question_details, hough_bubbles, contour_filled, student_number=None, student_details=None):
    ...
    if student_number and student_details:
        print(f"\nSTUDENT NUMBER SECTION (Format: Number-Number-Letter-Number-Number-Number-Number):")
        ...
        for pos, details in student_details.items():
            ...
            if len(filled_options) == 1:
                ...
                status = f"{', '.join(filled_options)} ← MULTIPLE SELECTIONS"
            else:
                status = "UNANSWERED"
    ...
    if multiple_selections > 0:
        print(f"⚠️  {multiple_selections} questions have multiple selections - review needed")
    
    if contour_only_count > both_methods_count * 0.3:
        print("⚠️  High number of contour-only detections - consider adjusting Hough parameters")
    
    if high_confidence_answers < answered_questions * 0.8:
        print("⚠️  Low high-confidence detection rate - consider image quality improvement")
...
def process_mcq_with_student_section(image_path):
    """Main processing pipeline for MCQ sheet with student number section"""
    # Load image
    original_img, img, gray = load_image(image_path)
    
    # Define column boundaries for standard MCQ sheet
    ...
    # Detection pipeline
    print("Step 1: Detecting bubbles with Hough Circles...")
    hough_bubbles = detect_hough_circles(gray)
    
    print("Step 2: Detecting filled areas with contour analysis...")
    # Detect in answer columns
    ...
    print("Step 3: Combining detection methods...")
    combined_bubbles = combine_detections(hough_bubbles, contour_filled)
    
    print("Step 4: Organizing into questions...")
    ...
    print("Step 5: Generating visualization and report...")
...
def export_results(answers, details, statistics, student_number=None, student_details=None, filename='mcq_results.json'):
    ...
    if student_number and student_details:
        ...
        results_export['student_details'] = student_details
...
if __name__ == "__main__":
    # Process MCQ sheet with student number section
    image_path = 'Solutions/MCQ_600dpi_2016/clean.jpg'
    ...
    export_results(answers, details, statistics, student_number, student_details)